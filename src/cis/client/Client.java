...
/**
 * This class is the client side for a server based on UDP/IP. The client
 * communicates with the error simulator at the specified port. Last edited
 * January 30th, 2018
 * 
...
public class Client {
	...
	public void processRequest() {
		...
		if (this.request == Request.READ) {
			ReceiverHandler receiverHandler = new ReceiverHandler(this.socket, address, Resources.errorSimulatorPort,
					this.filePath, this.directory, Resources.CLIENT);
			receiverHandler.process();

		} else if (this.request == Request.WRITE) {

			SenderHandler senderHandler = new SenderHandler(this.socket, address, Resources.errorSimulatorPort,
					this.filePath, Resources.CLIENT);
	 ...
	 *         Returns false if the request is a write and the file does not
	 *         exist.
	 */
